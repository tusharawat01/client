...
export default function MapComponent({
  ...
  function parseKMLCoordinates(kmlText) {
    ...
    for (let i = 0; i < placemarks.length; i++) {
      const coordinatesElement =
        placemarks[i].getElementsByTagName("coordinates")[0];
  ...
  return (
    <div
      className={`z-40 h-full w-full rounded-lg overflow-hidden 
    ...
    >
      ...
      <MapContainer
        center={[18.941842351894458, 73.02567770787]}
        zoom={13}
      ...
      >
        ...
        {loadedKml?.map((kml, k) => {
          return kml?.kml?.map((url, j) => <KmlComponent url={url} key={j} />);
        })}
