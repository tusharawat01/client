...
export default function WindmillProjectcard({ k, project }) {
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        router.push(
          `/clientSpecial/${currentUser?.name}/inspection?id=${project?._id}`
        );
      }}
    ...
    >
      ...
      <div className="flex flex-col gap-1 px-4 py-2 mt-1 gap-2 group-hover:bg-gray-900 transition-all duration-200 ease-in-out">
        <h1 className="text-md leading-none font-semibold text-gray-100">
          {project?.name}
        </h1>
        <h1 className="text-sm leading-none font-semibold text-gray-100">
          {project?.type}
        </h1>
