...
import {getAllImageWithTags,getAllGroupsWithId,fetchThermalImageByCoordinates} from '../../utils/ApiRoutes';
...
export default function AnalyseTab({
	...
	const [thermalZoomEnable,setThermalZoomEnable] = useState(true);
	...
	const fetchAllGroups = async() => {
		...
		// console.log(data,currentProject?.group)
		if(data.status){
	...
	const fetchThermalImage = async() => {
		...
		const {data} = await axios.post(fetchThermalImageByCoordinates,{
			latitude:currentImage?.exif_data?.latitude,
			longitude:currentImage?.exif_data?.longitude,
			CreateDate:currentImage?.exif_data?.CreateDate
		})
		if(data?.status){
			setThermalImage(data?.file[0]?.url)
		}
	}

	useEffect(()=>{
		if(thermalImage && !openMap){
		    const thermalImageContainer = document.getElementById('thermalImageContainer')

		  	if(thermalZoomEnable){
		  		zoomInstance = panzoom(thermalImageContainer,{
				  maxZoom: 10,
				  minZoom: 1
				});
		  	}else{
	...
	return (
		<div className="w-full overflow-auto h-full	flex flex-col justify-between">
			...
			pt-[70px] gap-5 p-5 overflow-auto`}>
				...
				{
					!openMap && thermalImage &&
					<div className="w-[50%] h-full overflow-hidden">
						<div id="thermalImageContainer" className="w-full h-full flex items-start">
							<img src={thermalImage} alt=""
