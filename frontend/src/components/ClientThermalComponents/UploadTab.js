...
import {createFolder,updateFilesInFolder,updateSolarProjectFolders,getFoldersById,
	createFile,getFilesByFolderId,getVideoFoldersById,createVideoFolder,getFolderName,uploadVideo,
	updateSolarProjectThermalFolders} from '../../utils/ApiRoutes';
...
export default function UploadTab({project,setProject,setCurrentTab,setCloseHeader}) {
	...
	const [uploadThermal,setUploadThermal] = useState(false);
    ...
    const foldersCreated = async(foldersCreatedData,loadedFilesInFolderId) => {
    	...
    	if(project?.folders){
	    	...
	    	if(uploadThermal){
	    		const {data} = await axios.post(updateSolarProjectThermalFolders,{thermalFolders:folders,id:project?._id});
	    		if(data?.status){
	    		...
	    		const {data} = await axios.post(updateSolarProjectFolders,{folders,id:project?._id});
	    		if(data?.status){
	    			setProject(data?.project);
	    			startUploadingFiles(data?.project,loadedFilesInFolderId);
					setCreatingFolder(false);
	...
	useEffect(()=>{
		if(project?.folders?.length > 0 || project?.thermalFolders?.length > 0){
			fetchFolders([...project?.folders,...project?.thermalFolders]);
	...
	return(
		<div className="w-full pt-[100px] px-5 max-w-6xl mx-auto">
			...
			ease-in-out z-50`}>
				...
				h-full w-full flex flex-col overflow-y-auto relative">
					...
					uploadThermal={uploadThermal} setUploadThermal={setUploadThermal}
					/>
				</div>
			</div>
			...
			{
				project?.folders?.length > 0 || project?.thermalFolders?.length > 0 ? 
				(
