...
import {getFilesByFolderId,updateFileTags,createTagGroup,
	updateGroupsInSolarProject,getTagGroups,updateTagsInGroup} from '../../utils/ApiRoutes'
...
export default function ImageTagTab({
	...
	useEffect(()=>{
		fetchFolders([...currentProject?.folders,...currentProject?.thermalFolders])
	},[])
	...
	const updateGroupsInSolarProjectFunc = async(tagGroup) => {
		
		let tempgroups = currentProject?.groups ? [...currentProject?.groups,tagGroup?._id] :  [tagGroup?._id];
		
		const {data} = await axios.post(updateGroupsInSolarProject,{
			groups:tempgroups,
	...
	const createTagGroupFunc = async() => {
		...
		if(data.status){
			updateGroupsInSolarProjectFunc(data?.tagGroup)
		}
	}
