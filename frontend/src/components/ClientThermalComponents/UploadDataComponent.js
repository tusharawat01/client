...
export default function UploadDataComponent({
	...
	setClearLoadedFiles,currentFileTab,uploadThermal,setUploadThermal
}) {
				...
				<div className="flex flex-col gap-3 px-5 py-3">
					<div className="flex flex-col gap-1 mb-2">
						<div className="flex items-center gap-2">
							<button onClick={()=>{
								setUploadThermal(false)
							}} className={`px-6 py-1 transition-all duration-200 ease-in-out 
							${uploadThermal ? 'hover:bg-gray-800' : 'bg-blue-500'} rounded-lg text-white`}>
								RGB
							</button>
							<button onClick={()=>{
								setUploadThermal(true)
							}} className={`px-6 py-1 transition-all duration-200 ease-in-out 
							${uploadThermal ? 'bg-blue-500' : 'hover:bg-gray-800'} rounded-lg text-white`}>
								Thermal
							</button>
						</div>
					...
					text-gray-200`}>
						{currentFileTab === 'image' ? uploadThermal ? 'THERMAL IMAGES' : 'RGB IMAGES' : videoLoading ? 'LOADING VIDEOS...' : 'VIDEO' }
					</h1>
					<div className="w-full bg-gray-700 h-[1px]"/>
