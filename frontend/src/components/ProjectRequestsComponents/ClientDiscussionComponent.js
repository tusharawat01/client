...
export default function ClientDiscussionComponent({ requests }) {
  ...
  useEffect(() => {
    if (requests?.length > 0) {
      const idx = requests.findIndex(
        (req) => req.status === "Client discussion"
      );
      const filteredRequests = requests.filter((req) => {
        if (req.status.toLowerCase() === "client discussion") return true;
        return false;
      });
