...
export default function FinalisedProjectComponent({ requests }) {
  ...
  useEffect(() => {
    if (requests.length > 0) {
      const idx = requests.findIndex(
        (req) => req.status === "Finalising Request"
      );
      const filteredRequests = requests.filter((req) => {
        if (req.status.toLowerCase() === "finalising request") return true;
        return false;
      });
