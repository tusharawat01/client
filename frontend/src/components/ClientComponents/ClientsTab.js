...
import AddClientByClient from "./AddClientByClient";
  ...
  return (
    <div className="w-full h-full">
      <AddClientByClient
        ...
        clientIndustry={currentClient?.clientIndustry}
        organizationName={currentClient?.organizationName}
        organizationType={currentClient?.organizationType}
        organizationId={currentClient?.organizationId}
        loadClients={loadClients}
      />
      <div
        className="w-full px-5 py-3 flex md:flex-row flex-col 
      ...
      >
        <form className="flex items-center md:w-[70%] w-full gap-3 ">
          <div className="border-[1px] border-gray-300/60 px-3 py-1 rounded-md">
            <input
              type="text"
              className="bg-transparent w-full outline-none ring-none 
      ...
      <div
        className={`fixed ${
          removeClientReveal ? "w-full h-full" : "h-[0px] w-[0px]"
        } overflow-hidden
      ...
      >
        <div className="p-3 bg-white border-[1px] border-gray-300 rounded-lg flex flex-col">
          <h1 className="text-xl font-cursive text-black mr-5">
            Are you sure? Remove{" "}
            <span className="text-blue-500">{removeClient?.name}</span> from
            client role.
          </h1>
          <div className="flex items-center mt-3 justify-end gap-5">
            <button
              onClick={(e) => setRemoveClientReveal(false)}
            ...
            <button
              onClick={() => {
                if (!loading) removeClientFunc();
              }}
              className={`${
                loading ? "bg-red-300/50 animate-pulse" : "bg-red-600"
