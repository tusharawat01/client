...
export default function ProjectTabMapCard({kml,j,k,setCurrentKmlFile,currentKmlFile}) {
	// body...
	...
	useEffect(()=>{
		...
		map = new Map({
		  ...
		  target: document.getElementById(`map-${j}${k}`),
		  view: view,
		  controls: []
		});
		...
		group justify-between gap-1 aspect-square w-full`} key={j} >
			...
			overflow-hidden h-full rounded-md w-full transition-scale duration-300 ease-in-out `}>
				<div className="h-full w-full">
					<div id={`map-${j}${k}`} className=" h-full w-full" />
				</div>
			</div>
		</div>
