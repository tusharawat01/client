...
export default function ProjectTabMapCard({kml,o,setCurrentKmlFile,currentKmlFile}) {
	// body...
	...
	useEffect(()=>{
		...
		map = new Map({
		  ...
		  target: document.getElementById(`siteCard-${o}`),
		  view: view,
		  controls: []
		});
	...
	return (
		...
		group justify-between gap-1 aspect-square w-full`} key={o} >
			<div className={`bg-white shadow-md 
			overflow-hidden h-full rounded-md w-full transition-scale duration-300 ease-in-out `}>
				<div className="h-full w-full">
					<div id={`siteCard-${o}`} className=" h-full w-full" />
				</div>
			</div>
		</div>
