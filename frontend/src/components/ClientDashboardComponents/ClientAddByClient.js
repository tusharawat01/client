...
export default function AddClient({addClientOpen,setAddClientOpen,
	...
	const [clientRole,setClientRole] = useState('manager')
	const [path,setPath] = useState('');
	const [url,setUrl] = useState('https://ik.imagekit.io/d3kzbpbila/thejashari_cktnTcxvv?updatedAt=1694788139311');
	...
	const createClientNow = async(imageUrl) => {
		...
		if(!data2?.data?.status){
			const {data} = await axios.post(registerUserRouteByAdmin,{
				...
				organizationType,clientIndustry,organizationRole:['client',clientRole],
				email:clientMail,roles:['User','Client'],
	...
	const upload = () =>{
		if(url.length > 2){
			if(pathCheck(path) || url === 'https://ik.imagekit.io/d3kzbpbila/thejashari_cktnTcxvv?updatedAt=1694788139311'){
				setLoading(true)
				// e.preventDefault();
				imagekit.upload({
			    file : url, //required
				...
				toast('Please Select an Image',toastOptions);
				setLoading(false)				
			}  
		}
	}
	...
	return (
		...
		ease-in-out`}>
			<div className="relative m-auto lg:w-[40%] md:w-[60%] sm:w-[80%] sm:max-h-[92%] w-full sm:rounded-lg 
			overflow-y-auto	border-[1px] border-gray-200/50 bg-white flex flex-col">
				...
				border-b-[1px] border-gray-300 gap-3 px-2">
					<div 
					onClick={()=>{
						setAddClientOpen(false);
					}}
					...
					<h1 className="text-xl select-none text-black  font-semibold">Add Team Member</h1>
				</div>
				...
				<div className="flex flex-col mt-3 px-4">
					...
					<div className="px-3">
						<div className="w-full flex items-center px-5 py-3 border-[1.3px] border-violet-500 
						rounded-lg mt-3 gap-5">
							<ImUserTie className="text-violet-500 h-7 w-7"/>
							<select value={clientRole}
							onChange={(e)=>setClientRole(e.target.value)}
							className="w-full bg-transparent outline-none ring-none font-semibold text-lg 
							text-black placeholder:text-gray-500 placeholder:font-normal placeholder:text-md"
							>
