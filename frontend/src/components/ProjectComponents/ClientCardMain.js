...
import {LiaIndustrySolid,LiaIdCard} from 'react-icons/lia';
import {useRouter} from 'next/navigation';
import {GoOrganization} from 'react-icons/go'
import {BsFillPersonFill} from 'react-icons/bs';

export default function ClientCardMain({client,j}) {
	// body...
	...
	return (
		<div key={j} className="bg-white flex flex-col rounded-md shadow-md px-3 py-4 shadow-gray-300/50">
			...
			<div className="mt-3 w-full flex flex-col">
				...
				<p className="text-xs text-gray-500"><span className="text-gray-600">Project field</span>: {client?.clientIndustry?.length > 1 ? 
				client?.clientIndustry.map((industry,j)=>(
					<span key={j} >{industry}{j+1 >= client?.clientIndustry?.length ? '' : ', ' }</span>
				))
				:
				client?.clientIndustry?.map((industry,j)=>(
				...
				<div className="mt-3 flex flex-col gap-[6px]">
					<h1 className="text-gray-800 flex items-center gap-2 text-xs">
						<LiaIdCard className="h-4 w-4 text-purple-400"/><span className="text-xs font-normal text-gray-700">{client?.organizationRole?.map((role,j)=>(
							<span key={j} className="">{role}{j+1 >= client?.organizationRole?.length ? '' : ', ' }</span>
						))}</span>
					</h1>
					<h1 className="text-gray-800 flex items-center gap-2 text-xs">
						<GoOrganization className="h-4 w-4 text-purple-400"/>{client?.organizationName}
					</h1>
					<h1 className="text-gray-800 flex items-center gap-2 text-xs">
						<IoMdCall className="h-4 w-4 text-purple-400"/>{client?.number}
					</h1>
					<h1 className="text-gray-800 flex items-center gap-2 text-xs">
						<GrMail className="h-4 w-4 text-purple-400"/>{client?.email}
					</h1>
					<h1 className="text-gray-800 flex items-center gap-2 text-xs">
						<LiaIndustrySolid className="h-4 w-4 text-purple-400"/>{client?.organizationType}
					</h1>
				</div>
