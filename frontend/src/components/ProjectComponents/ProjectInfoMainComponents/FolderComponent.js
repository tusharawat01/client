...
export default function FolderComponent({folder,uploadingFilesStarted,showMarkers,
	...
	async function exportFiles(allFiles) {
		setLoading(true);
		const imageUrls = await allFiles?.map((file)=>file.url);
		console.log(imageUrls);
        try {
            const response = await axios.post(host+'/download-images', {
                imageUrls:imageUrls 
            }, {
	            responseType: 'blob' // Specify the responseType as 'blob'
	        });
            console.log(response.data);
            // Create a temporary URL for the downloaded zip file
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `${folder?.name}-${project?.name}.zip`);
	...
	return (
		<tr className="bg-gray-50/40 cursor-pointer group border-b-[1px] border-gray-300 hover:bg-gray-100/60">
			<td onClick={showMap} className="px-4 py-2 text-center" >
				<h1 className="text-sm font-semibold text-gray-800">{folder?.name}</h1>
			</td>
			<td onClick={showMap} className="px-4 py-2 text-center" >
				<h1 className="text-sm font-semibold text-gray-800">{folder?.folderCreatedDate}</h1>
			</td>
			<td onClick={showMap} className="px-4 py-2 text-center" >
				<h1 className="text-sm font-semibold text-gray-800">{folder?.userDetails?.name}</h1>
			</td>
			<td onClick={showMap} className="px-4 py-2 text-center ">
				<h1 className="text-sm font-semibold text-gray-800">{allFiles?.length}</h1>
			</td>
			<td className="px-4 py-2 text-center relative ">
				...
				duration-200 ease-in-out flex items-center justify-center">
					<MdOutlineChevronRight className="h-5 w-5 text-gray-700"/>
				</div>
				{
					loading ? 
					...
					:
					<h1 
					onClick={()=>exportFiles(allFiles)}
					className="text-sm font-semibold w-full text-center text-gray-800 hover:text-blue-500 flex 
