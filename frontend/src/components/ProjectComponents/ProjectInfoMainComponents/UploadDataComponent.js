		...
		<div className={`flex flex-col mt-14 relative ${loadedFiles.length > 0 ? 'w-full' : 'md:w-[60%] w-full'} mx-auto`}>
			{
				...
				<MapComponent showMap={showMap} setShowMap={setShowMap} loadedFiles={loadedFiles}
			...
			}
			{
				...
				<div className="flex flex-col gap-3 px-5 py-3 overflow-hidden">
					<h1 className="flex items-center md:flex-row flex-col gap-5 justify-between font-semibold leading-none text-gray-800">
						<input
			        type="file"
			        id="uploadfileInput"
			        name="fileInput"
			      ...
			      />
						...
						<div className="flex items-center gap-3">
							<div className="flex items-center gap-1 group cursor-pointer">
								<AiOutlinePlusCircle 
								className={`h-5 w-5 text-gray-600 ${loading ? 'cursor-not-allowed' : 'cursor-pointer group-hover:text-blue-500'}`}/>
								<p 
								onClick={()=>{if(!loading) document.getElementById('uploadfileInput').click()}}
								className={`${!loading ? 'group-hover:text-blue-500' : 'text-gray-500 cursor-not-allowed'}`}>Add Files</p>
							</div>
							{
					...
					<div className="w-full flex items-center sm:flex-row flex-col gap-5 justify-between">
						<h1 className="text-gray-600 flex items-center text-sm gap-1">
					...
					<div className="overflow-x-auto w-full border-[1px] border-gray-300 rounded-lg">
						<table className="table-fixed rounded-lg sm:w-full ">
							<thead className="rounded-t-lg" >
						      <tr className="bg-gray-100 hover:bg-gray-200/70 text-gray-800 text-black px-5 py-3 border-b-[1px] border-gray-300">
						    ...
						    <tbody>
							{
								...
								loadedFiles.map((file,k)=>(
									<FileCard file={file} key={k} k={k} currentFileTab={currentFileTab}
									/>
								))
								:
			      ...
			      />
					...
					<div className="flex flex-col w-full p-5">
						 ...
						 rounded-lg border-dashed border-sky-400 hover:border-blue-500 p-5 pt-2 text-center">
					      <input {...getInputProps()} />
					      <AiOutlineCloudUpload className="h-[110px]  w-[110px] text-blue-700 hover:text-blue-600"/>
					      <p>Drag &apos;n&apos; drop some files and folders here</p>
					    </div>
					</div>
