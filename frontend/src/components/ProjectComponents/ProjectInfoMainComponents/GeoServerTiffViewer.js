...
export default function GeoServerTiffViewer({store,mapBasedDeliverables,
	selectedDate,project,setProject,setOpenGeoServerViewer}) {
 	...
 	{/*{
				...
				<LayersControl position="bottomleft">
...

				    {
				    	mapBasedDeliverables?.map((deliverable,k)=>{
				    			...
				    			return (
					    		<LayersControl.Overlay name={`${deliverable?.store?.split("-")[1]} - ${deliverable?.date}`} key={k}>
								    <WMSTileLayer
 		...
 		flex flex-col bg-black`}>
			...
			<div className="flex justify-between items-center gap-2 pb-2">
				...
				<div onClick={()=>{setOpenGeoServerViewer(false)}} className="cursor-pointer 
