...
export default function ProcessedDataViewer({
  ...
  //           (deliverable) => deliverable?.deliverableType
  //         ),
  //         ...currentProject?.deliverables?.map(
  //           (deliverable) => deliverable?.deliverableType
  //         ),
  ...
  //       console.log("available deliverables : ", availableDeliverables);
  //     }
  //   }, [currentProject]);

  useEffect(() => {
    if (currentProject?.deliverables?.length > 0) {
      const deliverables = currentProject.deliverables.map(
        (deliverable) => deliverable?.deliverableType
      );

      const filteredDeliverables = [...new Set(deliverables)];

      setAvailableDeliverables(filteredDeliverables);
      setSelectedDeliverable(filteredDeliverables[0]);

      console.log("available deliverables : ", filteredDeliverables);
    }
  }, [currentProject]);

  useEffect(() => {
    let filteredDelivereables = currentProject?.deliverables?.filter(
      (deliverable) => {
    ...
    if (filteredDelivereables?.length > 0) {
      let dates = [
        ...filteredDelivereables?.map((deliverable) => deliverable?.date),
      ];
  ...
  useEffect(() => {
    if (selectedDeliverable && selectedDate) {
      let filteredDelivereables = currentProject?.deliverables?.filter(
        (deliverable) => {
          if (
            deliverable?.deliverableType === selectedDeliverable &&
            deliverable?.date === selectedDate
          )
            return true;
          return false;
        }
  ...
  return (
    <div className="w-full h-[90%]">
      <div
        className="px-4 flex items-center justify-between py-3 border-b-[1px] border-gray-700
      ...
      >
        <div className="flex items-center">
          ...
          <div
            onClick={() => {
              setOpenDeliverablesOptions(!openDeliverablesOptions);
            }}
          ...
          >
            <h1
              className="text-gray-200 text-md  
						border-gray-500"
            >
              {selectedDeliverable || "No Deliverable Available"}
            </h1>
            <FaAngleDown
              className={`text-gray-200 ${
                openDeliverablesOptions ? "rotate-180" : "rotate-0"
              } transition-all 
            ...
            <div
              className={`${
                openDeliverablesOptions
                  ? "h-auto border-[1px] "
                  : "h-[0px] border-[0px] overflow-hidden"
              } 
            ...
            >
              {availableDeliverables?.map((deliverable, k) => (
                <div
                  ...
                  key={k}
                  className="px-3 py-1 flex items-center gap-2 hover:gap-3 border-b-[1px] 
									hover:bg-gray-800 border-gray-700 transition-all duration-200 ease-in-out"
                >
                  <p className="text-md font-normal whitespace-nowrap text-gray-200">
                    {deliverable}
                  </p>
          ...
          <div
            onClick={() => {
              setOpenDatesOptions(!openDatesOptions);
            }}
            className="relative p-1 hover:rounded-lg border-[1px] hover:border-gray-500 
          ...
          >
            ...
            <FaAngleDown
              className={`text-gray-200 ${
                openDatesOptions ? "rotate-180" : "rotate-0"
              } transition-all 
						duration-200 ease-in-out h-4 w-4`}
            />
      ...
      <div className="w-full flex z-20 h-full">
        <div className="w-[20%] gap-2 bg-[#292929] px-3 flex flex-col py-3">
          {showDeliverables?.length > 0 && (
            <h3 className="text-md text-gray-200 mb-2">
          ...
          {/* {showDeliverables?.map((deliverable, k) => (
            <div
              ...
              className={`px-2 w-full cursor-pointer 
							${
                openDeliverable?.store === deliverable?.store
                  ? "bg-blue-600 border-gray-800 hover:bg-blue-500 hover:border-gray-800"
          ...
          {currentProject?.deliverables?.map((deliverable, k) => (
            <div
              key={k}
              onClick={() => {
                if (openDeliverable?.store === deliverable?.store) {
                  ...
                  setOpenDeliverable(deliverable);
                  setSelectedDeliverable(deliverable?.deliverableType); // Update selected type
                }
              }}
              className={`px-2 w-full cursor-pointer 
      ${
