...
const save = (
  ...
  const updateProjectSitesFunc = async (
    kml,
    siteData,
    router,
    currentProject,
    ...
    const { data } = await axios.post(
      `${updateProjectSites}?industry=${currentProject?.industry}`,
      {
...
export default function GeoServerTiffViewer({
  store,
  mapBasedDeliverables,
  selectedDate,
  project,
  setProject,
  setOpenGeoServerViewer,
}) {
  ...
  const [openAnnotationDetailsEnterTab, setOpenAnnotationDetailsEnterTab] =
    useState(false);
  ...
  const getBoundingBox = async () => {
    if (!store) return;
    const url = `${GeoServerURL}/geoserver/rest/workspaces/a2a/coveragestores/${store}/coverages/${store}.json`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (bbox) {
          const extent = [bbox.minx, bbox.miny, bbox.maxx, bbox.maxy].map(
  ...
  const getBoundingBox2 = async (selectedStore) => {
    ...
    const url = `${GeoServerURL}/geoserver/rest/workspaces/a2a/coveragestores/${selectedStore}/coverages/${selectedStore}.json`;
  ...
  useEffect(() => {
    ...
    //       url: '${GeoServerURL}/geoserver/a2a/wms',
    //       params: {
    //         'LAYERS':`a2a:${store}`
    //       },
    //     }),
  ...
  const onCreate = (e) => {
    ...
    if (layerType === "polygon" || layerType === "rectangle") {
      ...
      layerData.latlngs = latLngs;
      layerData.area = L.GeometryUtil.geodesicArea(latLngs);
      layerData.squareKm = layerData.area / 1000000;
    } else if (layerType === "polyline") {
      const latLngs = layer.getLatLngs();
      layerData.latlngs = latLngs;
      layerData.km = L.GeometryUtil.length(layer) / 1000;
    ...
    setMapLayers((layers) => [...layers, layerData]);
  ...
  const onEdit = (e) => {
    ...
    editedLayers.forEach((layer) => {
      const updatedLayerData = {
        ...
        type: layer.type,
        geoJson: layer.toGeoJSON(),
        latlngs: layer.getLatLngs ? layer.getLatLngs()[0] : undefined,
        radius: layer.getRadius ? layer.getRadius() : undefined,
      };
      setMapLayers((layers) =>
        layers.map((layerData) =>
          layerData.id === updatedLayerData.id ? updatedLayerData : layerData
        )
      );
    });
  };
  ...
  const saveMapLayers = async () => {
    ...
    const { data } = await axios.post(
      `${updateMapLayersInProject}?industry=${project?.industry}`,
      {
  ...
  return (
    <div className="h-full w-full flex z-0 relative">
      <div
        className={`absolute ${
          openSidebar ? "-top-[100%]" : savedMapLayers?.length > 0 && "top-2"
        } 
      ...
      <div
        className={` ${
          savedMapLayers?.length > 0 && openSidebar ? "w-[75%]" : "w-[100%]"
        } h-full transition-all duration-200 ease-in-out z-10 overflow-hidden relative`}
      >
        {savedMapLayers?.length > 0 && (
          ...
          >
            <div className="flex">
              <div
                onClick={() =>
                  downloadKml([...savedMapLayers, ...mapLayers], setData)
                }
              ...
              >
                ...
                <p className="text-xs hidden group-hover:block text-gray-700">
                  KML
                </p>
              </div>
            </div>
        ...
        {showIssueDetails && (
          <div className="absolute z-50 h-full w-full bg-black/40 p-2 flex items-center justify-center top-0 left-0">
            <div className="flex flex-col rounded-lg w-[350px] max-h-[90%] bg-white overflow-y-auto">
              <div
                className="bg-black px-4 py-2 border-b-[1px] border-gray-300 
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="p-1 rounded-lg border-[1px] border-gray-300 w-full bg-gray-100">
                    <input
                      ...
                      value={showIssueDetails?.issueName}
                      disabled
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="p-1 rounded-lg border-[1px] border-gray-300 w-full bg-gray-100">
                    <input
                      ...
                      value={showIssueDetails?.issueType}
                      disabled
                      className="bg-transparent outline-none text-black w-full"
                    />
                  </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="p-1 rounded-lg border-[1px] border-gray-300 w-full bg-gray-100">
                    <textarea
                      ...
                      value={showIssueDetails?.issueRemarks}
                      disabled
          ...
          <div className="absolute z-50 h-full w-full bg-black/40 p-2 flex items-center justify-center top-0 left-0">
            <div className="flex flex-col rounded-lg w-[350px] max-h-[90%] bg-white overflow-y-auto">
              ...
              <div className="flex flex-col gap-3 p-3">
                ...
                <button
                  onClick={saveTheAnnotationDetails}
                  className="text-gray-100 bg-black rounded-lg w-full px-4 py-1 mx-auto"
                >
                  Save
                </button>
              </div>
        ...
        >
            ...
            {mapLayers?.map((shape) => (
              <div key={shape?.id}>
                ...
                shape.type === "polyline" ? (
                  ...
                  <SVGOverlay
                    attributes={{ stroke: "transparent" }}
                    bounds={[Object.values(shape?.latlng)]}
                  >
                    <text
                      x="50%"
                      y="50%"
                  ...
                  return (
                    ...
                    >
                      <Polygon
                        ...
                        key={shape.id}
                        positions={shape.latlngs.map((latlng) => [
                          latlng.lat,
                          latlng.lng,
                        ])}
                        color="blue"
                  ...
                  return (
                    ...
                    >
                      <Rectangle
                        ...
                        bounds={[
                          [
                            shape.bounds._southWest.lat,
                            shape.bounds._southWest.lng,
                          ],
                          [
                            shape.bounds._northEast.lat,
                            shape.bounds._northEast.lng,
                          ],
                        ]}
                        onClick={() => {
                          handleFeatureClick(shape, k);
          ...
          <LayersControl position="bottomleft">
            ...
            <LayersControl.Overlay name="Mapbox Map">
              <TileLayer
                url={`https://api.mapbox.com/v4/mapbox.satellite/{z}/{x}/{y}@2x.jpg90?access_token=${mapboxAccessToken}`}
            ...
            {mapBasedDeliverables?.map((deliverable, k) => {
              return (
                <LayersControl.Overlay
                  name={`${deliverable?.store?.split("-")[1]} - ${
                    deliverable?.date
                  }`}
                  key={k}
                >
      ...
      <div
        className={` ${
          savedMapLayers?.length > 0 && openSidebar
            ? "w-[25%] overflow-y-auto   scrollbar scrollbar-thin px-2 py-3"
      ...
      >
        <div className="flex justify-between items-center gap-2 pb-2">
          <div className="flex items-center gap-2">
            <div
              ...
              className="hover:bg-gray-800/70 cursor-pointer p-1 rounded-full"
            >
          ...
          <div
            onClick={() => {
              setOpenGeoServerViewer(false);
            }}
            className="cursor-pointer 
				p-2 rounded-full hover:bg-gray-300/60 transition-all 
			 	duration-200 ease-in-out"
          >
            <RxCross2 className="text-gray-100 h-5 w-5" />
          </div>
        </div>
        <div
          className={`flex flex-col gap-3 ${
            mapLayers?.length > 0 ? "py-3" : "p-0"
          }`}
        >
        ...
        <div className="w-full flex flex-col mt-2 items-center gap-1">
          ...
          >
            <button
              className={`w-full py-2 flex justify-center items-center gap-1 rounded-lg bg-gray-900 border-[1px] border-gray-700 
					text-gray-200 text-sm transition-all duration-200 ease-in-out
					${
            savedMapLayers?.length > 0
              ? "hover:border-sky-500 opacity-[100%] hover:text-sky-500"
