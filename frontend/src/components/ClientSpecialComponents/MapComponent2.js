...
export default function MapComponent2({
	...
	return(
		...
		bg-gray-700/60 p-5 flex flex-col`}>
				<MapContainer
			    ...
			    <div className="w-full px-5 py-1 gap-2 bg-[#232323] rounded-b-lg flex items-center justify-end">
			    	<div 
						onClick={()=>setShowMap(false)}
						className={`tranisition-all duration-200 ease-in-out right-5 cursor-pointer transition-all duration-200 ease-in-out 
						p-1 rounded-full rounded-lg text-gray-200 hover:text-gray-100 z-50`}>
							Close
						</div>

						<div 
						onClick={()=>exportMarkersAsKML()}
						className={`tranisition-all duration-200 ease-in-out right-5 cursor-pointer transition-all duration-200 ease-in-out hover:bg-gray-800/60 
						bg-gray-900 px-3 py-1 rounded-lg text-sm border-[1px] border-gray-700 text-gray-200 hover:text-white z-50 flex items-center gap-2`}>
