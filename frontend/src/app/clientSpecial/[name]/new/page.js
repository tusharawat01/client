...
import {getTowerProject,createTowerProject,updateTowerProjectsId} from '../../../../utils/ApiRoutes';
...
const options = [
	'Lattice Self-Supported Horizontal Structure',
	'Lattice Self-Supported Delta Structure',
	'Vertical Structure',
	'Lattice Guyed Single-Column Delta Structure',
	'Lattice H-Frame',
	'Lattice Guyed-Vee',
	'Single-Shaft Modified Delta Structure',
...
export default function Home() {
	...
	const createProjectWithImage = async(image) => {
		const {data} = await axios.post(createTowerProject,{
		...
		if(data.status){
			console.log(data);
			updateUserTowerProjectId(data?.tower?._id)
		}else{
			setLoading(false)
			alert("Something went wrong! cant create project")
		}
	}

	const updateUserTowerProjectId = async(towerDBId) => {
		const ids = [...currentUser?.towerProjectsId,towerDBId];
		console.log(ids);
		const {data} = await axios.post(updateTowerProjectsId,{
			id:currentUser?._id,
			towerProjectsId:ids
		})
		if(data?.status){
	...
	return (
		<main className="w-[100%] h-[100vh] overflow-y-scroll pb-[50px]">
			<div className="fixed top-0 w-full px-5 flex items-center 
			justify-between py-3 bg-[#292929] z-30">
				<div className="flex items-center gap-4">
					...
					<h1 className="md:text-xl text-md font-semibold text-gray-200">New Tower Project</h1>
				</div>
			...
			<div className="flex md:flex-row flex-col px-5 py-3 gap-10 max-w-6xl mx-auto pt-[100px]">
				...
				border-gray-700 px-5 py-3 md:w-[50%] w-full">
					...
					<div className="px-4 py-2 mt-2 rounded-md border-[1px] border-gray-600 focus-within:border-sky-600 bg-gray-900/30 hover:bg-gray-900/60 w-full">
						...
						placeholder="Enter tower name or ID" value={nameOrID} onChange={(e)=>setNameOrID(e.target.value)}
						/>
					</div>
					<h1 className="text-md font-semibold text-gray-200 mt-5">Area <span className="text-red-500">*</span></h1>
					<div className="px-4 py-2 mt-2 rounded-md border-[1px] border-gray-600 focus-within:border-sky-600 bg-gray-900/30 hover:bg-gray-900/60 w-full">
						<input type="text" className="placeholder:text-gray-500 text-gray-200 bg-transparent outline-none w-full" 
						placeholder="Enter tower name or ID" value={area} onChange={(e)=>setArea(e.target.value)}
						/>
					</div>
					...
					<h1 className="text-md font-semibold text-gray-200 mt-5">Tower type <span className="text-red-500">*</span></h1>
					<div className="px-4 py-2 mt-2 rounded-md border-[1px] border-gray-600 focus-within:border-sky-600 bg-gray-900/30 hover:bg-gray-900/60 w-full">
